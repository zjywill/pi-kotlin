...
if [[ ! -f "$TS_ROOT/packages/ai/dist/index.js" ]]; then
  npm --prefix "$TS_ROOT/packages/ai" run build:offline >&2
fi
if [[ ! -f "$TS_ROOT/packages/agent/dist/index.js" ]]; then
  npm --prefix "$TS_ROOT/packages/agent" run build >&2
fi
...
PI_TYPESCRIPT_ROOT="$TS_ROOT" NODE_NO_WARNINGS=1 \
  node --experimental-strip-types migration/oracle/session-jsonl.ts \
  | normalize_json > "$TMP_DIR/typescript.json"
./gradlew -q :pi-coding-agent:sessionJsonlOracle \
